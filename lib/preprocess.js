...
function preprocessor(src, context, opts, noRestoreEol) {
  ...
  if (opts.type.foreach) {
    rv = replaceRecursive(rv, opts.type.foreach, function(startMatches, endMatches, include) {
      var variable = (startMatches[1] || '').trim();
      var forParams = variable.split(' ');
      if (forParams.length === 3) {
        var contextVar = forParams[2];
        var vars = forParams[0].split(',');
        var keyName, valueName;
        if (vars.length === 2) {
          keyName = vars[0];
          valueName = vars[1];
        } else {
          valueName = forParams[0];
        }
        var array = getDeepPropFromObj(context, contextVar);
        var eachArr;
        ...
        return Object.keys(eachArr).reduce(function(stringBuilder, arrKey){
          ...
          var extendedContext = copy(context);
          if (keyName) {
            extendedContext[keyName] = arrKey;
            extendedContext[valueName] = arrEntry;
          } else {
            extendedContext[valueName] = arrEntry;
          }
          var recursedInclude = preprocessor(include, extendedContext, opts, true);
          return stringBuilder + recursedInclude;
        }, '');
      } else {
        return '';
      }
